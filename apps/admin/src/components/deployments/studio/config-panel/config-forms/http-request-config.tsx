...
interface Props { config: HttpRequestConfig; onChange: (config: HttpRequestConfig) => void }
...
export function HttpRequestConfigForm({ config, onChange }: Props) {
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="space-y-1.5">
                <Label className="text-sm text-brand-main-200">Body</Label>
                <textarea
                    value={config.body ?? ''}
            ...
            <div className="space-y-1.5">
                <Label className="text-sm text-brand-main-200">Response Variable</Label>
                <Input
                    value={config.responseVariable ?? ''}
                    onChange={(e) => onChange({ ...config, responseVariable: e.target.value })}
                    className="bg-brand-main-700/50 border-brand-main-500 text-white light:text-brand-main-50"
                    placeholder="response"
                />
                <p className="text-xs text-brand-main-400">Variable name to store the response data in the workflow context.</p>
            </div>
        </div>
    )
}
