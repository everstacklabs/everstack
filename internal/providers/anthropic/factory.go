...
func init() {
	factory.Default.Register("anthropic", func(in factory.AggregatedInput) (gw.Provider, error) {
		var apiKey string
		...
		if len(in.APIKeys) > 0 {
			...
			if err != nil {
				...
				return nil, fmt.Errorf("no available Anthropic API keys: %w", err)
			}
		...
		// so the logging layer sees the post-compaction view sent to
		// the provider, while tracing sees the actual provider call.
		// Disabled configs short-circuit cheaply; non-allowlist
		// providers also no-op.
