...
export const CodeBlock = ({
  ...
  return (
    <CodeBlockContext.Provider value={{ value, onValueChange, data }}>
      <div
        className={cn('size-full overflow-hidden rounded-md border border-brand-main-500', className)}
        {...props}
      />
...
export const CodeBlockHeader = ({
  ...
  <div
    className={cn(
      'flex flex-row items-center border-b border-brand-main-500 bg-secondary p-1',
      className
    )}
    {...props}
  />
...
export const CodeBlockContent = ({
  ...
  useEffect(() => {
    ...
    const loadHighlightedCode = async () => {
      try {
        const html = await highlightClientCode(children, language, themes);
